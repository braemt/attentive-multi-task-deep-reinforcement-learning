...
parser.add_argument('-e', '--env-id', type=str, default="grid-worlds-v0,grid-worlds-v1",
                    help="Environment ids")
parser.add_argument('-l', '--log-dir', type=str, default="/tmp/attentive",
...
def create_commands(session, env_id, logdir, shell='bash', mode='tmux'):
    # for launching the TF workers and for launching tensorboard
    env_ids = str(env_id).split(",")
    tasks = len(env_ids)

    num_workers = tasks
    while num_workers + tasks <= 24:
        num_workers += tasks
    make_logdir = logdir

    base_cmd = [sys.executable, 'worker.py', '--log-dir', logdir, '--env-id', env_id, '--num-workers', str(num_workers)]
    ...
    tensorboard_logdir = make_logdir
    cmds_map += [new_cmd(session, "tb", ["tensorboard", "--logdir", tensorboard_logdir, "--port", "12345"], mode, logdir, shell)]
    if mode == 'tmux':
    ...
    notes = []
    cmds = [
        "mkdir -p {}/code".format(make_logdir),
        "echo {} {} > {}/cmd.sh".format(sys.executable,
                                        ' '.join([shlex_quote(arg) for arg in sys.argv if arg != '-n']),
                                        make_logdir),
    ]
    if mode == 'nohup' or mode == 'child':
        cmds += ["echo '#!/bin/sh' >{}/kill.sh".format(make_logdir)]
        notes += ["Run `source {}/kill.sh` to kill the job".format(make_logdir)]
    if mode == 'tmux':
        ...
        notes += ["Use `tail -f {}/*.out` to watch process output".format(make_logdir)]
    ...
    if mode == 'tmux':
        cmds += [
            ...
            "kill -9 $( lsof -i:12222-{} -t ) > /dev/null 2>&1".format(num_workers + 12222),
...
def run():
    ...
    cmds, notes = create_commands("a3c", args.env_id, args.log_dir, mode=args.mode)
    if args.dry_run:
