...
from gym_pycolab.prefab_parts import drapes
from gym_pycolab.prefab_parts import sprites
    ...
    def assertMachinima(self, engine, frames,
        ...
        Args:
          ...
          frames: a sequence of n-tuples, where `n >= 2`. The first element in each
              ...
              portraying the observation we expect the `play` method to return. Any
              further elements of the tuple are stored in the engine's `Plot` object
              under the key `'machinima_args'`. These are commonly used to pass
              expected values to `assertEqual` tests to callables provided via
              `pre_updates` and `post_updates`.
